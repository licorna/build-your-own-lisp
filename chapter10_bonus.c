...
lval* builtin_cons(lval* a) {
  LASSERT(a, a->count == 2,
          "Function 'cons' needs exactly 2 arguments!");
  LASSERT(a, a->cell[1]->type == LVAL_QEXPR,
          "Function 'cons' needs qexpr as second argument!");

  lval* q = a->cell[1];

  /* Increase count of qexpr. */
  q->count += 1;
  /* And increase its memory. */
  q->cell = realloc(q->cell, sizeof(lval*) * (q->count));
  /* Push contents to the "right". */
  memmove(&q->cell[1], &q->cell[0], sizeof(lval*) * (q->count-1));
  /* Set the first cell to the new value. */
  q->cell[0] = a->cell[0];
...
lval* builtin_init(lval* a) {
  ...
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR,
          "Function 'init' passed wrong arguments!");

  lval* lst = a->cell[0];
  /* Remove last element from list. */
  lval* last = lval_pop(lst, lst->count - 1);
  /* Don't forget to remove this element. */
  lval_del(last);
  return lst;
}
